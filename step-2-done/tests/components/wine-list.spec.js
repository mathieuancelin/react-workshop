...
describe('<WineList />', () => {
  ...
  it('doit afficher un style différent si un vin est sélectionné', () => {
    const wrapper = shallow(
      <WineList wines={wines} selected={wines[1]} onWineChange={null} />
    );
    const children = wrapper.get(0).props.children;
    expect(wrapper.equals(
      <div>
        <div {...children[0].props}>{wines[0].name}</div>
        <div {...children[1].props} style={{ fontWeight: 'bold', padding: 16, backgroundColor: 'lightGrey' }}>{wines[1].name}</div>
        <div {...children[2].props}>{wines[2].name}</div>
      </div>
    )).to.equal(true);
  });
  ...
  it('doit gérer le click sur un vin', () => {
    ...
    const wrapper = shallow(
      <WineList wines={wines} selected={wines[0]} onWineChange={onWineChange} />
    );
    const children = wrapper.get(0).props.children;
    expect(wrapper.equals(
      <div>
        <div {...children[0].props} style={{ fontWeight: 'bold', padding: 16, backgroundColor: 'lightGrey' }}>{wines[0].name}</div>
        <div {...children[1].props}>{wines[1].name}</div>
        <div {...children[2].props}>{wines[2].name}</div>
      </div>
    )).to.equal(true);
